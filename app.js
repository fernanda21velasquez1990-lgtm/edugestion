...
(() => {
  ...
  const SECTION_ID='section-control-estudio';
  let secciones=[];
  let alumnos=[];
  let stats=null;
  let actividades=[];
  let contexto={ano:'',seccion:'',turno:'',lapso:'1er Lapso'};
  let ultimoZip=null;
  ...
  const storeKey=()=>`edugestion_control_estudio_v1_${userKey()}`;
  const configKey=()=>`edugestion_control_estudio_contacto_v1_${userKey()}`;
  ...
  function estilos(){
    if(document.getElementById('edu-control-estudio-styles')) return;
    const st=document.createElement('style');st.id='edu-control-estudio-styles';st.textContent=`
      .ce-hero{padding:22px;border-radius:22px;background:linear-gradient(135deg,#233876,#316cb0);color:#fff;margin-bottom:18px;box-shadow:0 12px 30px rgba(35,56,118,.18)}
      .ce-hero small{font-weight:900;text-transform:uppercase;letter-spacing:.08em;opacity:.92}.ce-hero h2{margin:6px 0 8px;font-size:1.6rem}.ce-hero p{margin:0;max-width:980px;opacity:.95}
      ...
      @media(max-width:900px){.ce-grid{grid-template-columns:1fr 1fr}.ce-metrics{grid-template-columns:repeat(2,1fr)}.ce-activity-head{grid-template-columns:1fr 1fr}.ce-contact{grid-template-columns:1fr}}
      @media(max-width:600px){.ce-grid{grid-template-columns:1fr}.ce-metrics{grid-template-columns:1fr 1fr}.ce-activity-head{grid-template-columns:1fr}}
    `;document.head.appendChild(st);
  }

  function asegurarUI(){
    estilos();const nav=document.getElementById('app-nav'),main=document.getElementById('app-main');if(!nav||!main)return;
    let tab=document.getElementById(TAB_ID);
    ...
    if(!sec){sec=document.createElement('section');sec.id=SECTION_ID;sec.className='hidden';sec.innerHTML=`
      ...
      <div class="ce-card"><h3>1. Selecciona el lapso y la sección</h3><div class="ce-grid">
        ...
        <div class="ce-field"><label>Desde</label><input id="ce-desde" type="date"></div>
        <div class="ce-field"><label>Hasta</label><input id="ce-hasta" type="date"></div>
      </div><div class="ce-actions"><button id="ce-cargar" class="ce-btn primary"><i class="fa-solid fa-rotate"></i> Cargar estudiantes y asistencia</button></div><div id="ce-status" class="ce-muted"></div></div>
      <div class="ce-card"><h3>2. Actividades y notas del lapso</h3><div class="ce-actions"><button id="ce-add" class="ce-btn soft"><i class="fa-solid fa-plus"></i> Agregar actividad</button></div><div id="ce-activities"><div class="ce-empty">Primero carga una sección.</div></div><div id="ce-weight-msg"></div></div>
      <div class="ce-card"><h3>3. Cuadro final por sección</h3><div id="ce-metrics" class="ce-metrics"></div><div id="ce-final"><div class="ce-empty">El cuadro aparecerá al cargar la sección.</div></div></div>
      <div class="ce-card"><h3>4. Entrega a Control de Estudio</h3><div class="ce-contact"><div class="ce-field"><label>Correo de Control de Estudio</label><input id="ce-email" type="email" placeholder="controlestudio@institucion.edu"></div><div class="ce-field"><label>WhatsApp de Control de Estudio</label><input id="ce-whatsapp" placeholder="Ej.: 584141234567"></div></div>
        <div class="ce-actions"><button id="ce-zip" class="ce-btn primary"><i class="fa-solid fa-file-zipper"></i> Descargar ZIP</button><button id="ce-share" class="ce-btn blue"><i class="fa-solid fa-share-nodes"></i> Compartir ZIP</button><button id="ce-mail" class="ce-btn soft"><i class="fa-solid fa-envelope"></i> Correo</button><button id="ce-wa" class="ce-btn soft"><i class="fa-brands fa-whatsapp"></i> WhatsApp</button></div>
  ...
  async function abrir(tab){
    const sec=document.getElementById(SECTION_ID);if(!sec)return;
    if(typeof window.cambiarPestana==='function')window.cambiarPestana(tab,sec);else{document.querySelectorAll('#app-nav .nav-item').forEach(x=>{x.classList.toggle('is-active',x===tab);x.setAttribute('aria-selected',x===tab?'true':'false')});document.querySelectorAll('#app-main > section').forEach(x=>x.classList.toggle('hidden',x!==sec));}
    if(!secciones.length)await cargarSecciones();
  }

  async function cargarSecciones(){
    const sel=document.getElementById('ce-seccion');if(!sel)return;sel.innerHTML='<option value="">Cargando…</option>';
    try{const api=window.EDUGESTION_API_REQUEST;if(typeof api!=='function')throw new Error('La conexión con EduGestión no está disponible.');const r=await api('obtenerDatosIniciales');const horarios=Array.isArray(r.horarios)?r.horarios:[];const map=new Map();horarios.forEach(x=>{const ano=String(x.ano||'').trim(),seccion=String(x.seccion||'').trim(),turno=String(x.turno||'').trim();if(ano&&seccion)map.set([ano,seccion,turno].join('|'),{ano,seccion,turno});});if(!map.size&&window.profesorActual?.seccion){const ano=String(window.profesorActual?.ano||window.profesorActual?.grado||'').trim();const seccion=String(window.profesorActual.seccion||'').trim(),turno=String(window.profesorActual.turno||'').trim();if(seccion)map.set([ano,seccion,turno].join('|'),{ano,seccion,turno});}secciones=[...map.values()].sort((a,b)=>(a.ano+a.seccion).localeCompare(b.ano+b.seccion,'es'));sel.innerHTML='<option value="">Selecciona una sección</option>'+secciones.map(s=>`<option value="${esc([s.ano,s.seccion,s.turno].join('|'))}">${esc(s.ano||'Curso')} · Sección ${esc(s.seccion)}${s.turno?' · '+esc(s.turno):''}</option>`).join('');}
    catch(err){sel.innerHTML='<option value="">No se pudieron cargar las secciones</option>';document.getElementById('ce-status').textContent=err.message||'Error de conexión.';}
  }
  ...
  function archivoBase(){return `Control_Estudio_${(contexto.ano||'Curso').replace(/\s+/g,'_')}_Seccion_${(contexto.seccion||'X').replace(/\s+/g,'_')}_${contexto.lapso.replace(/\s+/g,'_')}`}
  function generarCsv(){const headers=['N°','Estudiante','ID','Año/Grado','Sección','Turno','Días asistidos','Inasistencias','Tardanzas','Justificadas',...actividades.flatMap(a=>[`${a.nombre} - Entregó`,`${a.nombre} - Nota /20`,`${a.nombre} - Ponderación %`]),'Actividades entregadas','Nota final /20'];const lines=[headers.map(csvEscape).join(',')];alumnos.forEach((al,i)=>{const as=resumenAsistenciaAlumno(al),rn=resumenNotasAlumno(al);const row=[i+1,al.nombre||al.nombreCompleto||'',al.cedula||al.id||'',contexto.ano,contexto.seccion,contexto.turno,as.presentes,as.ausentes,as.tardanzas,as.justificadas];actividades.forEach(a=>{const r=a.registros?.[String(al.id)]||{};row.push(r.entrego==='Si'?'Sí':'No',r.nota===''||r.nota==null?'':r.nota,a.ponderacion||0)});row.push(`${rn.entregadas}/${rn.total}`,rn.nota===null?'':rn.nota.toFixed(2));lines.push(row.map(csvEscape).join(','));});return '\uFEFF'+lines.join('\r\n');}
  function generarHtml(){const filas=alumnos.map((al,i)=>{const as=resumenAsistenciaAlumno(al),rn=resumenNotasAlumno(al);return`<tr><td>${i+1}</td><td>${esc(al.nombre||al.nombreCompleto||'')}</td><td>${as.presentes}</td><td>${as.ausentes}</td><td>${as.tardanzas}</td><td>${rn.entregadas}/${rn.total}</td><td>${rn.nota===null?'—':rn.nota.toFixed(2)}</td></tr>`}).join('');const acts=actividades.map(a=>`<li><strong>${esc(a.nombre)}</strong> · ${esc(a.fecha||'Sin fecha')} · ${n(a.ponderacion)}%</li>`).join('');return`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${esc(archivoBase())}</title><style>body{font-family:Arial,sans-serif;margin:28px;color:#1d2733}h1{font-size:22px}h2{font-size:15px;margin-top:24px}table{width:100%;border-collapse:collapse;margin-top:12px}th,td{border:1px solid #bfc8d3;padding:7px;font-size:12px}th{background:#eef3f7}small{color:#667}</style></head><body><h1>Cuadro final de Notas y Control de Estudio</h1><p><strong>Docente:</strong> ${esc(window.profesorActual?.nombre||'')}<br><strong>Materia:</strong> ${esc(window.profesorActual?.materia||'')}<br><strong>Curso:</strong> ${esc(contexto.ano)} · Sección ${esc(contexto.seccion)} · ${esc(contexto.turno)}<br><strong>Lapso:</strong> ${esc(contexto.lapso)}<br><strong>Periodo de asistencia:</strong> ${esc(document.getElementById('ce-desde')?.value||'')} al ${esc(document.getElementById('ce-hasta')?.value||'')}</p><h2>Actividades del lapso</h2><ol>${acts||'<li>Sin actividades registradas.</li>'}</ol><h2>Cuadro por sección</h2><table><thead><tr><th>N°</th><th>Estudiante</th><th>Días asistidos</th><th>Inasistencias</th><th>Tardanzas</th><th>Actividades entregadas</th><th>Nota final /20</th></tr></thead><tbody>${filas}</tbody></table><p><small>Generado por EduGestión el ${new Date().toLocaleString('es-ES')}.</small></p></body></html>`;}
  function generarResumenTxt(){return `EDUGESTIÓN - CONTROL DE ESTUDIO\nDocente: ${window.profesorActual?.nombre||''}\nMateria: ${window.profesorActual?.materia||''}\nCurso: ${contexto.ano} · Sección ${contexto.seccion} · ${contexto.turno}\nLapso: ${contexto.lapso}\nPeriodo: ${document.getElementById('ce-desde')?.value||''} al ${document.getElementById('ce-hasta')?.value||''}\nEstudiantes: ${alumnos.length}\nActividades: ${actividades.length}\n\nEl archivo CSV contiene el detalle completo de asistencia, entregas y calificaciones. El archivo HTML contiene el cuadro final listo para imprimir.`;}
  ...
  function zipStore(files){const enc=new TextEncoder(),out=[],central=[];let offset=0;const now=new Date();const dostime=((now.getHours()&31)<<11)|((now.getMinutes()&63)<<5)|((Math.floor(now.getSeconds()/2))&31);const dosdate=(((now.getFullYear()-1980)&127)<<9)|(((now.getMonth()+1)&15)<<5)|(now.getDate()&31);files.forEach(f=>{const name=enc.encode(f.name),data=typeof f.data==='string'?enc.encode(f.data):f.data,crc=crc32(data);const local=[];u32(local,0x04034b50);u16(local,20);u16(local,0x0800);u16(local,0);u16(local,dostime);u16(local,dosdate);u32(local,crc);u32(local,data.length);u32(local,data.length);u16(local,name.length);u16(local,0);local.push(...name);const start=offset;out.push(...local,...data);offset+=local.length+data.length;const cen=[];u32(cen,0x02014b50);u16(cen,20);u16(cen,20);u16(cen,0x0800);u16(cen,0);u16(cen,dostime);u16(cen,dosdate);u32(cen,crc);u32(cen,data.length);u32(cen,data.length);u16(cen,name.length);u16(cen,0);u16(cen,0);u16(cen,0);u16(cen,0);u32(cen,0);u32(cen,start);cen.push(...name);central.push(...cen)});const centralOffset=offset;out.push(...central);const end=[];u32(end,0x06054b50);u16(end,0);u16(end,0);u16(end,files.length);u16(end,files.length);u32(end,central.length);u32(end,centralOffset);u16(end,0);out.push(...end);return new Blob([new Uint8Array(out)],{type:'application/zip'});}
  function construirZip(){if(!alumnos.length)throw new Error('Carga primero una sección.');const base=archivoBase();return{blob:zipStore([{name:`${base}.csv`,data:generarCsv()},{name:`${base}.html`,data:generarHtml()},{name:'LEEME.txt',data:generarResumenTxt()}]),name:`${base}.zip`};}
  function descargar(blob,name){const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000)}
  function generarZip(descarga){try{guardarContacto();ultimoZip=construirZip();if(descarga)descargar(ultimoZip.blob,ultimoZip.name);toast('ZIP de Control de Estudio generado correctamente.','success');return ultimoZip}catch(err){toast(err.message||'No se pudo generar el ZIP.','error');return null}}
  async function compartirZip(){const z=generarZip(false);if(!z)return;const file=new File([z.blob],z.name,{type:'application/zip'});const data={title:'Control de Estudio',text:`Cuadro final ${contexto.lapso} · ${contexto.ano} Sección ${contexto.seccion}`,files:[file]};try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share(data);return;}descargar(z.blob,z.name);toast('Tu navegador no permite compartir el ZIP directamente. Se descargó para que lo adjuntes.','info');}catch(err){if(err?.name!=='AbortError'){descargar(z.blob,z.name);toast('Se descargó el ZIP para que puedas adjuntarlo manualmente.','info');}}}
  function mensaje(){return `Buenos días. Adjunto cuadro final de ${contexto.lapso}, ${contexto.ano} Sección ${contexto.seccion}, materia ${window.profesorActual?.materia||''}, docente ${window.profesorActual?.nombre||''}. Archivo generado desde EduGestión para Control de Estudio.`}
  function abrirCorreo(){const z=generarZip(false);if(!z)return;descargar(z.blob,z.name);const c=guardarContacto(),subject=encodeURIComponent(`Control de Estudio - ${contexto.ano} Sección ${contexto.seccion} - ${contexto.lapso}`),body=encodeURIComponent(`${mensaje()}\n\nAdjuntar archivo descargado: ${z.name}`);window.location.href=`mailto:${encodeURIComponent(c.email||'')}?subject=${subject}&body=${body}`;}
  function abrirWhatsApp(){const z=generarZip(false);if(!z)return;descargar(z.blob,z.name);const c=guardarContacto(),tel=String(c.whatsapp||'').replace(/\D/g,''),text=encodeURIComponent(`${mensaje()}\n\nAdjunto: ${z.name}`);window.open(`https://wa.me/${tel}?text=${text}`,'_blank','noopener');}
  function toast(msg,type='info'){if(typeof window.mostrarToast==='function')window.mostrarToast(msg,type,'Control de Estudio');else alert(msg)}
...
/* =========================================================
   ...
   - Resume asistencia, entregas, calificaciones y observación del lapso.
   - Usa las notas ya registradas en Notas y Control de Estudio.
...
(() => {
  ...
  const SECTION_ID='section-boletin-estudiante';
  let secciones=[], alumnos=[], stats=null, actividades=[];
  let contexto={ano:'',seccion:'',turno:'',lapso:'1er Lapso'};
  ...
  const userKey=()=>String(window.profesorActual?.id||window.profesorActual?.usuario||window.profesorActual?.email||'sin_usuario').replace(/[^a-z0-9_-]/gi,'_');
  const controlKey=()=>`edugestion_control_estudio_v1_${userKey()}`;
  const obsKey=()=>`edugestion_boletines_observaciones_v1_${userKey()}`;
  const periodoKey=()=>[contexto.ano,contexto.seccion,contexto.turno,contexto.lapso].join('|');
  function loadJSON(k){try{return JSON.parse(localStorage.getItem(k)||'{}')||{}}catch(_){return{}}}
  function saveJSON(k,v){try{localStorage.setItem(k,JSON.stringify(v))}catch(_){}}
  function cargarNotas(){const all=loadJSON(controlKey());const p=all[periodoKey()]||{};actividades=Array.isArray(p.actividades)?p.actividades:[];}
  function idAlumno(al){return String(al?.id||al?.cedula||al?.documento||al?.nombre||al?.nombreCompleto||'').trim();}
  function getObs(al){const all=loadJSON(obsKey());return String(all[periodoKey()]?.[idAlumno(al)]||'');}
  function setObs(al,val){const all=loadJSON(obsKey());all[periodoKey()]=all[periodoKey()]||{};all[periodoKey()][idAlumno(al)]=val;saveJSON(obsKey(),all);}

  function estilos(){
    if(document.getElementById('edu-boletin-styles'))return;
    const st=document.createElement('style');st.id='edu-boletin-styles';st.textContent=`
      .be-hero{padding:22px;border-radius:22px;background:linear-gradient(135deg,#233876,#316cb0);color:#fff;margin-bottom:18px;box-shadow:0 12px 30px rgba(35,56,118,.18)}
      .be-hero small{font-weight:900;text-transform:uppercase;letter-spacing:.08em;opacity:.92}.be-hero h2{margin:6px 0 8px;font-size:1.6rem}.be-hero p{margin:0;max-width:980px;opacity:.95}
      .be-card{background:var(--card-bg,#fff);border:1px solid var(--border-color,#dfe7ee);border-radius:18px;padding:16px;margin-bottom:14px}.be-card h3{margin:0 0 12px;font-size:1.05rem}
      .be-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.be-field label{display:block;font-size:.76rem;font-weight:900;opacity:.7;margin:0 0 5px}.be-field input,.be-field select,.be-obs{width:100%;padding:10px 11px;border:1px solid var(--border-color,#d7e0e8);border-radius:10px;background:var(--input-bg,#fff);color:inherit}
      .be-actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:12px}.be-btn{border:0;border-radius:11px;padding:10px 13px;font-weight:900;cursor:pointer}.be-btn.primary{background:#234f91;color:#fff}.be-btn.blue{background:#285ca8;color:#fff}.be-btn.soft{background:#edf3fb;color:#234f91}
      .be-summary{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;margin:10px 0 14px}.be-summary article{padding:12px;border-radius:13px;background:#f1f6fc;border:1px solid #dce9f8}.be-summary strong{display:block;font-size:1.2rem;color:#234f91}.be-summary span{font-size:.72rem;font-weight:850;opacity:.72}
      .be-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.be-student{border:1px solid var(--border-color,#dfe7ee);border-radius:16px;padding:14px;background:var(--card-bg,#fff)}.be-student h4{margin:0 0 4px}.be-student small{opacity:.68}.be-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:11px 0}.be-stats div{padding:8px;border-radius:10px;background:#f3f7fc;text-align:center}.be-stats b{display:block;color:#234f91}.be-stats span{font-size:.68rem;font-weight:800;opacity:.72}.be-obs{min-height:72px;resize:vertical;margin-top:8px}.be-empty{padding:24px;text-align:center;border:1px dashed var(--border-color,#ccd8e5);border-radius:14px;opacity:.75}.be-muted{font-size:.78rem;opacity:.7;margin-top:8px}
      @media(max-width:900px){.be-grid{grid-template-columns:1fr 1fr}.be-summary{grid-template-columns:repeat(2,1fr)}.be-list{grid-template-columns:1fr}}
      @media(max-width:600px){.be-grid{grid-template-columns:1fr}.be-summary{grid-template-columns:1fr 1fr}.be-stats{grid-template-columns:1fr 1fr}}
    `;document.head.appendChild(st);
  }
  ...
  function asegurarUI(){
    ...
    if(!sec){sec=document.createElement('section');sec.id=SECTION_ID;sec.className='hidden';sec.innerHTML=`
      ...
      <div class="be-card"><h3>1. Selecciona lapso y sección</h3><div class="be-grid">
        ...
        <div class="be-field"><label>Desde</label><input id="be-desde" type="date"></div>
        <div class="be-field"><label>Hasta</label><input id="be-hasta" type="date"></div>
      </div><div class="be-actions"><button id="be-cargar" class="be-btn primary"><i class="fa-solid fa-rotate"></i> Cargar boletines</button><button id="be-zip" class="be-btn soft"><i class="fa-solid fa-file-zipper"></i> ZIP de toda la sección</button></div><div id="be-status" class="be-muted"></div></div>
      <div class="be-card"><h3>2. Resumen de la sección</h3><div id="be-summary" class="be-summary"></div><div class="be-field"><label>Buscar estudiante</label><input id="be-search" placeholder="Escribe nombre o cédula..."></div></div>
      <div class="be-card"><h3>3. Boletines individuales</h3><div id="be-list"><div class="be-empty">Selecciona una sección y pulsa “Cargar boletines”.</div></div></div>`;main.appendChild(sec);
      const hoy=new Date(),y=hoy.getFullYear(),m=String(hoy.getMonth()+1).padStart(2,'0'),d=String(hoy.getDate()).padStart(2,'0');sec.querySelector('#be-hasta').value=`${y}-${m}-${d}`;sec.querySelector('#be-desde').value=`${y}-${m}-01`;
      sec.querySelector('#be-cargar').addEventListener('click',cargar);sec.querySelector('#be-zip').addEventListener('click',descargarZipSeccion);sec.querySelector('#be-search').addEventListener('input',render);sec.querySelector('#be-list').addEventListener('input',e=>{const ta=e.target.closest('[data-be-obs]');if(ta){const al=alumnos.find(x=>idAlumno(x)===ta.dataset.beObs);if(al)setObs(al,ta.value);}});sec.querySelector('#be-list').addEventListener('click',e=>{const b=e.target.closest('[data-be-action]');if(!b)return;const al=alumnos.find(x=>idAlumno(x)===b.dataset.student);if(!al)return;if(b.dataset.beAction==='ver')verBoletin(al);if(b.dataset.beAction==='imprimir')imprimirBoletin(al);if(b.dataset.beAction==='html')descargarHtml(al);});
    }
  }
  ...
  function card(x){const al=x.al,as=x.as,no=x.no,id=esc(idAlumno(al));return`<article class="be-student"><h4>${esc(al.nombre||al.nombreCompleto||'Estudiante')}</h4><small>${esc(al.cedula||al.id||'Sin identificación')} · ${esc(contexto.ano)} · Sección ${esc(contexto.seccion)}</small><div class="be-stats"><div><b>${as.presentes}</b><span>Asistió</span></div><div><b>${as.ausentes}</b><span>Inasistencias</span></div><div><b>${no.entregadas}/${no.total}</b><span>Entregó</span></div><div><b>${no.nota===null?'—':no.nota.toFixed(2)}</b><span>Nota /20</span></div></div><label style="font-size:.72rem;font-weight:850;opacity:.72">Observación del lapso</label><textarea class="be-obs" data-be-obs="${id}" placeholder="Escribe una observación breve del estudiante...">${esc(getObs(al))}</textarea><div class="be-actions"><button class="be-btn soft" data-be-action="ver" data-student="${id}"><i class="fa-solid fa-eye"></i> Ver boletín</button><button class="be-btn primary" data-be-action="imprimir" data-student="${id}"><i class="fa-solid fa-print"></i> Imprimir</button><button class="be-btn soft" data-be-action="html" data-student="${id}"><i class="fa-solid fa-download"></i> Guardar HTML</button></div></article>`;}
  function htmlBoletin(al){const as=asistencia(al),no=notas(al),obs=getObs(al),acts=no.detalle.map((d,i)=>`<tr><td>${i+1}</td><td>${esc(d.nombre)}</td><td>${esc(d.fecha||'—')}</td><td>${d.entrego?'Sí':'No'}</td><td>${d.nota===null?'—':d.nota.toFixed(2)}</td><td>${d.ponderacion}%</td></tr>`).join('');return`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Boletín - ${esc(al.nombre||al.nombreCompleto||'Estudiante')}</title><style>body{font-family:Arial,sans-serif;margin:30px;color:#1d2733}.head{background:#234f91;color:white;padding:20px;border-radius:14px}h1{margin:0 0 6px;font-size:22px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:18px 0}.box{border:1px solid #d9e2ec;border-radius:12px;padding:12px}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.metrics div{background:#eef4fb;padding:12px;border-radius:10px;text-align:center}.metrics b{display:block;font-size:18px;color:#234f91}table{width:100%;border-collapse:collapse;margin-top:14px}th,td{border:1px solid #cbd5e1;padding:7px;font-size:12px}th{background:#eef4fb}.obs{margin-top:16px;padding:12px;border:1px solid #d9e2ec;border-radius:10px;min-height:55px}@media print{button{display:none}}</style></head><body><div class="head"><h1>Boletín de desempeño del lapso</h1><div>${esc(contexto.lapso)} · ${esc(window.profesorActual?.materia||'')}</div></div><div class="meta"><div class="box"><strong>Estudiante:</strong><br>${esc(al.nombre||al.nombreCompleto||'')}</div><div class="box"><strong>C.I./ID:</strong><br>${esc(al.cedula||al.id||'')}</div><div class="box"><strong>Curso:</strong><br>${esc(contexto.ano)} · Sección ${esc(contexto.seccion)} · ${esc(contexto.turno||'')}</div><div class="box"><strong>Docente:</strong><br>${esc(window.profesorActual?.nombre||'')}</div></div><div class="metrics"><div><b>${as.presentes}</b>Asistencias</div><div><b>${as.ausentes}</b>Inasistencias</div><div><b>${no.entregadas}/${no.total}</b>Actividades entregadas</div><div><b>${no.nota===null?'—':no.nota.toFixed(2)}</b>Nota final /20</div></div><h2 style="font-size:16px;margin-top:20px">Actividades del lapso</h2><table><thead><tr><th>N°</th><th>Actividad</th><th>Fecha</th><th>Entregó</th><th>Nota /20</th><th>Ponderación</th></tr></thead><tbody>${acts||'<tr><td colspan="6">Sin actividades registradas.</td></tr>'}</tbody></table><div class="obs"><strong>Observación del docente</strong><p>${esc(obs||'Sin observaciones registradas.')}</p></div><p style="font-size:11px;color:#667;margin-top:18px">Generado por EduGestión el ${new Date().toLocaleString('es-ES')}.</p></body></html>`;}
  function verBoletin(al){const w=window.open('','_blank');if(!w){toast('El navegador bloqueó la ventana del boletín.','warning');return;}w.document.open();w.document.write(htmlBoletin(al));w.document.close();}
  function imprimirBoletin(al){const w=window.open('','_blank');if(!w){toast('El navegador bloqueó la ventana de impresión.','warning');return;}w.document.open();w.document.write(htmlBoletin(al));w.document.close();setTimeout(()=>{w.focus();w.print();},350);}
  function archivoAlumno(al){return `Boletin_${String(al.nombre||al.nombreCompleto||'Estudiante').replace(/[^a-z0-9áéíóúñü_-]+/gi,'_')}_${contexto.lapso.replace(/\s+/g,'_')}.html`;}
  function descargarBlob(blob,name){const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),2500);}
  function descargarHtml(al){descargarBlob(new Blob([htmlBoletin(al)],{type:'text/html;charset=utf-8'}),archivoAlumno(al));}
  function csvResumen(){const lines=[['N°','Estudiante','ID','Asistencias','Inasistencias','Tardanzas','Actividades entregadas','Nota final /20','Observación']];alumnos.forEach((al,i)=>{const as=asistencia(al),no=notas(al);lines.push([i+1,al.nombre||al.nombreCompleto||'',al.cedula||al.id||'',as.presentes,as.ausentes,as.tardanzas,`${no.entregadas}/${no.total}`,no.nota===null?'':no.nota.toFixed(2),getObs(al)]);});return '\uFEFF'+lines.map(r=>r.map(v=>{const s=String(v??'');return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s}).join(',')).join('\r\n');}
  const crcTable=(()=>{const t=[];for(let i=0;i<256;i++){let c=i;for(let k=0;k<8;k++)c=(c&1)?0xedb88320^(c>>>1):c>>>1;t[i]=c>>>0}return t})();
  function crc32(bytes){let c=0xffffffff;for(const b of bytes)c=crcTable[(c^b)&255]^(c>>>8);return(c^0xffffffff)>>>0}function u16(a,v){a.push(v&255,(v>>>8)&255)}function u32(a,v){a.push(v&255,(v>>>8)&255,(v>>>16)&255,(v>>>24)&255)}
  function zipStore(files){const enc=new TextEncoder(),out=[],central=[];let offset=0;const now=new Date(),dostime=((now.getHours()&31)<<11)|((now.getMinutes()&63)<<5)|((Math.floor(now.getSeconds()/2))&31),dosdate=(((now.getFullYear()-1980)&127)<<9)|(((now.getMonth()+1)&15)<<5)|(now.getDate()&31);files.forEach(f=>{const name=enc.encode(f.name),data=typeof f.data==='string'?enc.encode(f.data):f.data,crc=crc32(data),local=[];u32(local,0x04034b50);u16(local,20);u16(local,0x0800);u16(local,0);u16(local,dostime);u16(local,dosdate);u32(local,crc);u32(local,data.length);u32(local,data.length);u16(local,name.length);u16(local,0);local.push(...name);const start=offset;out.push(...local,...data);offset+=local.length+data.length;const cen=[];u32(cen,0x02014b50);u16(cen,20);u16(cen,20);u16(cen,0x0800);u16(cen,0);u16(cen,dostime);u16(cen,dosdate);u32(cen,crc);u32(cen,data.length);u32(cen,data.length);u16(cen,name.length);u16(cen,0);u16(cen,0);u16(cen,0);u16(cen,0);u32(cen,0);u32(cen,start);cen.push(...name);central.push(...cen)});const centralOffset=offset;out.push(...central);const end=[];u32(end,0x06054b50);u16(end,0);u16(end,0);u16(end,files.length);u16(end,files.length);u32(end,central.length);u32(end,centralOffset);u16(end,0);out.push(...end);return new Blob([new Uint8Array(out)],{type:'application/zip'});}
  function descargarZipSeccion(){if(!alumnos.length){toast('Carga primero una sección.','warning');return;}const files=alumnos.map(al=>({name:archivoAlumno(al),data:htmlBoletin(al)}));files.unshift({name:'Resumen_seccion.csv',data:csvResumen()});const name=`Boletines_${(contexto.ano||'Curso').replace(/\s+/g,'_')}_Seccion_${(contexto.seccion||'X').replace(/\s+/g,'_')}_${contexto.lapso.replace(/\s+/g,'_')}.zip`;descargarBlob(zipStore(files),name);toast('ZIP de boletines generado correctamente.','success');}
  function status(msg,html=false){const el=document.getElementById('be-status');if(el){if(html)el.innerHTML=msg;else el.textContent=msg;}}
  function toast(msg,type='info'){if(typeof window.mostrarToast==='function')window.mostrarToast(msg,type,'Boletín por estudiante');else alert(msg)}
  function init(){asegurarUI();}
...
/* =========================================================
   EduGestión · FASE 15
   Cierre de lapso para Control de Estudio
   - Reúne cuadro final, asistencia, boletines y acta/resumen.
   - Genera un único ZIP por sección y lapso.
   - Mantiene el estilo azul general de EduGestión.
   ========================================================= */
(() => {
  const MARK='EDUGESTION_CIERRE_LAPSO_V1';
  if(window[MARK]) return;
  window[MARK]=true;
  const TAB_ID='tab-cierre-lapso';
  const SECTION_ID='section-cierre-lapso';
  let secciones=[], alumnos=[], stats=null, actividades=[];
  let contexto={ano:'',seccion:'',turno:'',lapso:'1er Lapso'};
  ...
  const controlKey=()=>`edugestion_control_estudio_v1_${userKey()}`;
  const obsKey=()=>`edugestion_boletines_observaciones_v1_${userKey()}`;
  const contactoKey=()=>`edugestion_control_estudio_contacto_v1_${userKey()}`;
  const periodoKey=()=>[contexto.ano,contexto.seccion,contexto.turno,contexto.lapso].join('|');
  const loadJSON=k=>{try{return JSON.parse(localStorage.getItem(k)||'{}')||{}}catch(_){return{}}};
  const saveJSON=(k,v)=>{try{localStorage.setItem(k,JSON.stringify(v))}catch(_){}};
  function idAlumno(al){return String(al?.id||al?.cedula||al?.documento||al?.nombre||al?.nombreCompleto||'').trim();}
  function cargarNotas(){const p=loadJSON(controlKey())[periodoKey()]||{};actividades=Array.isArray(p.actividades)?p.actividades:[];}
  function getObs(al){return String(loadJSON(obsKey())[periodoKey()]?.[idAlumno(al)]||'');}
  function contacto(){return loadJSON(contactoKey());}

  function estilos(){
    if(document.getElementById('edu-cierre-lapso-styles'))return;
    const st=document.createElement('style');st.id='edu-cierre-lapso-styles';st.textContent=`
      .cl-hero{padding:22px;border-radius:22px;background:linear-gradient(135deg,#233876,#316cb0);color:#fff;margin-bottom:18px;box-shadow:0 12px 30px rgba(35,56,118,.18)}
      .cl-hero small{font-weight:900;text-transform:uppercase;letter-spacing:.08em;opacity:.92}.cl-hero h2{margin:6px 0 8px;font-size:1.6rem}.cl-hero p{margin:0;max-width:980px;opacity:.95}
      .cl-card{background:var(--card-bg,#fff);border:1px solid var(--border-color,#dfe7ee);border-radius:18px;padding:16px;margin-bottom:14px}.cl-card h3{margin:0 0 12px;font-size:1.05rem}
      .cl-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.cl-field label{display:block;font-size:.76rem;font-weight:900;opacity:.7;margin:0 0 5px}.cl-field input,.cl-field select,.cl-field textarea{width:100%;padding:10px 11px;border:1px solid var(--border-color,#d7e0e8);border-radius:10px;background:var(--input-bg,#fff);color:inherit}
      .cl-actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:12px}.cl-btn{border:0;border-radius:11px;padding:10px 13px;font-weight:900;cursor:pointer}.cl-btn.primary{background:#234f91;color:#fff}.cl-btn.blue{background:#285ca8;color:#fff}.cl-btn.soft{background:#edf3fb;color:#234f91}
      .cl-summary{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;margin:10px 0}.cl-summary article{padding:12px;border-radius:13px;background:#f1f6fc;border:1px solid #dce9f8}.cl-summary strong{display:block;font-size:1.2rem;color:#234f91}.cl-summary span{font-size:.72rem;font-weight:850;opacity:.72}
      .cl-checks{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.cl-check{display:flex;gap:10px;align-items:flex-start;padding:12px;border:1px solid #dce7f4;border-radius:12px;background:#f8fbff}.cl-check input{margin-top:3px}.cl-check b{display:block;color:#234f91}.cl-check small{opacity:.7}
      .cl-preview{overflow:auto}.cl-table{width:100%;border-collapse:collapse;min-width:760px}.cl-table th,.cl-table td{padding:8px;border-bottom:1px solid var(--border-color,#e4e9ef);text-align:left;font-size:.8rem}.cl-table th{background:#f1f6fc;font-size:.72rem;text-transform:uppercase;letter-spacing:.04em}
      .cl-empty{padding:24px;text-align:center;border:1px dashed var(--border-color,#ccd8e5);border-radius:14px;opacity:.75}.cl-muted{font-size:.78rem;opacity:.72;margin-top:8px}
      @media(max-width:900px){.cl-grid{grid-template-columns:1fr 1fr}.cl-summary{grid-template-columns:repeat(2,1fr)}.cl-checks{grid-template-columns:1fr}}
      @media(max-width:600px){.cl-grid{grid-template-columns:1fr}.cl-summary{grid-template-columns:1fr 1fr}}
    `;document.head.appendChild(st);
  }
  ...
  function asegurarUI(){
    estilos();const nav=document.getElementById('app-nav'),main=document.getElementById('app-main');if(!nav||!main)return;
    let tab=document.getElementById(TAB_ID);
    if(!tab){tab=document.createElement('button');tab.id=TAB_ID;tab.type='button';tab.className='nav-item';tab.setAttribute('aria-selected','false');tab.dataset.title='Cierre de lapso';tab.dataset.description='Genera el paquete completo por sección para Control de Estudio.';tab.innerHTML='<i class="fa-solid fa-box-archive"></i><span>Cierre de lapso</span>';const ref=document.getElementById('tab-estadisticas');nav.insertBefore(tab,ref||null);tab.addEventListener('click',()=>abrir(tab));}
    let sec=document.getElementById(SECTION_ID);
    if(!sec){sec=document.createElement('section');sec.id=SECTION_ID;sec.className='hidden';sec.innerHTML=`
      <header class="cl-hero"><small><i class="fa-solid fa-box-archive"></i> Fase 15 · Control de Estudio</small><h2>Cierre de lapso</h2><p>Genera en un solo paquete el cuadro final de notas, resumen de asistencia, boletines individuales y acta del lapso por sección.</p></header>
      <div class="cl-card"><h3>1. Selecciona lapso y sección</h3><div class="cl-grid">
        <div class="cl-field"><label>Lapso</label><select id="cl-lapso"><option>1er Lapso</option><option>2do Lapso</option><option>3er Lapso</option></select></div>
        <div class="cl-field"><label>Sección / Año</label><select id="cl-seccion"><option value="">Cargando secciones…</option></select></div>
        <div class="cl-field"><label>Desde</label><input id="cl-desde" type="date"></div>
        <div class="cl-field"><label>Hasta</label><input id="cl-hasta" type="date"></div>
      </div><div class="cl-actions"><button id="cl-cargar" class="cl-btn primary"><i class="fa-solid fa-rotate"></i> Cargar cierre</button></div><div id="cl-status" class="cl-muted"></div></div>
      <div class="cl-card"><h3>2. Documentos incluidos</h3><div class="cl-checks">
        <label class="cl-check"><input id="cl-inc-cuadro" type="checkbox" checked><span><b>Cuadro final de notas</b><small>Notas, asistencia y entregas por estudiante.</small></span></label>
        <label class="cl-check"><input id="cl-inc-asistencia" type="checkbox" checked><span><b>Resumen de asistencia</b><small>Días asistidos, inasistencias y tardanzas.</small></span></label>
        <label class="cl-check"><input id="cl-inc-boletines" type="checkbox" checked><span><b>Boletines individuales</b><small>Un boletín HTML por cada estudiante.</small></span></label>
        <label class="cl-check"><input id="cl-inc-acta" type="checkbox" checked><span><b>Acta / resumen del lapso</b><small>Resumen institucional del cierre de la sección.</small></span></label>
      </div></div>
      <div class="cl-card"><h3>3. Resumen de la sección</h3><div id="cl-summary" class="cl-summary"></div><div id="cl-preview"><div class="cl-empty">Selecciona una sección y pulsa “Cargar cierre”.</div></div></div>
      <div class="cl-card"><h3>4. Entrega a Control de Estudio</h3><div class="cl-grid">
        <div class="cl-field"><label>Correo</label><input id="cl-email" type="email" placeholder="controlestudio@institucion.edu"></div>
        <div class="cl-field"><label>WhatsApp</label><input id="cl-wa" placeholder="Ej.: 584141234567"></div>
        <div class="cl-field" style="grid-column:span 2"><label>Observación general del cierre</label><textarea id="cl-obs" rows="3" placeholder="Observaciones generales para Control de Estudio..."></textarea></div>
      </div><div class="cl-actions"><button id="cl-zip" class="cl-btn primary"><i class="fa-solid fa-file-zipper"></i> Generar ZIP completo</button><button id="cl-share" class="cl-btn blue"><i class="fa-solid fa-share-nodes"></i> Compartir ZIP</button><button id="cl-mail" class="cl-btn soft"><i class="fa-solid fa-envelope"></i> Correo</button><button id="cl-whatsapp" class="cl-btn soft"><i class="fa-brands fa-whatsapp"></i> WhatsApp</button></div><div class="cl-muted">Si el navegador no permite adjuntar el ZIP automáticamente, EduGestión lo descargará y abrirá el mensaje listo para enviar.</div></div>`;main.appendChild(sec);
      const hoy=new Date(),y=hoy.getFullYear(),m=String(hoy.getMonth()+1).padStart(2,'0'),d=String(hoy.getDate()).padStart(2,'0');sec.querySelector('#cl-hasta').value=`${y}-${m}-${d}`;sec.querySelector('#cl-desde').value=`${y}-${m}-01`;
      sec.querySelector('#cl-cargar').addEventListener('click',cargar);sec.querySelector('#cl-zip').addEventListener('click',()=>generarZip(true));sec.querySelector('#cl-share').addEventListener('click',compartir);sec.querySelector('#cl-mail').addEventListener('click',abrirCorreo);sec.querySelector('#cl-whatsapp').addEventListener('click',abrirWhatsApp);
      const c=contacto();sec.querySelector('#cl-email').value=c.email||'';sec.querySelector('#cl-wa').value=c.whatsapp||'';
    }
  }

  async function abrir(tab){const sec=document.getElementById(SECTION_ID);if(!sec)return;if(typeof window.cambiarPestana==='function')window.cambiarPestana(tab,sec);else{document.querySelectorAll('#app-nav .nav-item').forEach(x=>{x.classList.toggle('is-active',x===tab);x.setAttribute('aria-selected',x===tab?'true':'false')});document.querySelectorAll('#app-main > section').forEach(x=>x.classList.toggle('hidden',x!==sec));}if(!secciones.length)await cargarSecciones();}
  async function cargarSecciones(){const sel=document.getElementById('cl-seccion');if(!sel)return;sel.innerHTML='<option value="">Cargando…</option>';try{const api=window.EDUGESTION_API_REQUEST;if(typeof api!=='function')throw new Error('La conexión con EduGestión no está disponible.');const r=await api('obtenerDatosIniciales');const horarios=Array.isArray(r.horarios)?r.horarios:[];const map=new Map();horarios.forEach(x=>{const ano=String(x.ano||'').trim(),seccion=String(x.seccion||'').trim(),turno=String(x.turno||'').trim();if(ano&&seccion)map.set([ano,seccion,turno].join('|'),{ano,seccion,turno});});secciones=[...map.values()].sort((a,b)=>(a.ano+a.seccion).localeCompare(b.ano+b.seccion,'es'));sel.innerHTML='<option value="">Selecciona una sección</option>'+secciones.map(s=>`<option value="${esc([s.ano,s.seccion,s.turno].join('|'))}">${esc(s.ano||'Curso')} · Sección ${esc(s.seccion)}${s.turno?' · '+esc(s.turno):''}</option>`).join('');}catch(err){sel.innerHTML='<option value="">No se pudieron cargar las secciones</option>';status(err.message||'Error de conexión.');}}
  async function cargar(){const v=document.getElementById('cl-seccion')?.value||'';if(!v){toast('Selecciona una sección.','warning');return;}const [ano,seccion,turno]=v.split('|');contexto={ano,seccion,turno,lapso:document.getElementById('cl-lapso')?.value||'1er Lapso'};const desde=document.getElementById('cl-desde')?.value||'',hasta=document.getElementById('cl-hasta')?.value||'';status('Cargando estudiantes, asistencia y notas…');try{const api=window.EDUGESTION_API_REQUEST;if(typeof api!=='function')throw new Error('La conexión con EduGestión no está disponible.');const [a,s]=await Promise.all([api('obtenerAlumnos',{ano,seccion,turno}),api('obtenerEstadisticasAsistencia',{fechaDesde:desde,fechaHasta:hasta,ano,seccion,turno})]);alumnos=Array.isArray(a.alumnos)?a.alumnos:[];stats=s||null;cargarNotas();render();status(`${alumnos.length} estudiantes · ${actividades.length} actividades cargadas para ${contexto.lapso}.`);}catch(err){status(err.message||'No se pudo cargar el cierre.');toast(err.message||'No se pudo cargar el cierre.','error');}}

  function asistencia(al){const id=idAlumno(al),raw=stats?.porAlumno?.[id]||stats?.alumnos?.find?.(x=>String(x.id||x.cedula||'')===id)||{};return{presentes:num(raw.presentes??raw.asistencias??raw.diasAsistidos),ausentes:num(raw.ausentes??raw.inasistencias),tardanzas:num(raw.tardanzas)};}
  function notas(al){let entregadas=0,suma=0,pesoTot=0;const detalle=actividades.map(a=>{const r=a.registros?.[String(al.id)]||a.registros?.[idAlumno(al)]||{};if(r.entrego==='Si')entregadas++;const nota=r.nota===''||r.nota==null?null:num(r.nota),peso=num(a.ponderacion);if(nota!==null&&peso>0){suma+=nota*peso;pesoTot+=peso;}return{nombre:a.nombre||'Actividad',fecha:a.fecha||'',ponderacion:peso,entrego:r.entrego==='Si',nota};});return{entregadas,total:actividades.length,nota:pesoTot?Math.round((suma/pesoTot)*100)/100:null,detalle};}
  function filas(){return alumnos.map((al,i)=>({i:i+1,al,as:asistencia(al),no:notas(al)}));}
  function render(){const rows=filas(),sum=document.getElementById('cl-summary'),prev=document.getElementById('cl-preview');if(!sum||!prev)return;const prom=rows.filter(x=>x.no.nota!==null);const promedio=prom.length?prom.reduce((s,x)=>s+x.no.nota,0)/prom.length:null;sum.innerHTML=`<article><strong>${rows.length}</strong><span>Estudiantes</span></article><article><strong>${actividades.length}</strong><span>Actividades</span></article><article><strong>${rows.reduce((s,x)=>s+x.as.presentes,0)}</strong><span>Asistencias</span></article><article><strong>${rows.reduce((s,x)=>s+x.as.ausentes,0)}</strong><span>Inasistencias</span></article><article><strong>${promedio===null?'—':promedio.toFixed(2)}</strong><span>Promedio /20</span></article>`;prev.innerHTML=rows.length?`<div class="cl-preview"><table class="cl-table"><thead><tr><th>N°</th><th>Estudiante</th><th>Asistió</th><th>Inasistencias</th><th>Entregó</th><th>Nota final /20</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${x.i}</td><td>${esc(x.al.nombre||x.al.nombreCompleto||'')}</td><td>${x.as.presentes}</td><td>${x.as.ausentes}</td><td>${x.no.entregadas}/${x.no.total}</td><td>${x.no.nota===null?'—':x.no.nota.toFixed(2)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="cl-empty">No hay estudiantes cargados.</div>';}
  function csvCuadro(){const lines=[['N°','Estudiante','ID','Días asistidos','Inasistencias','Tardanzas','Actividades entregadas','Nota final /20','Observación']];filas().forEach(x=>lines.push([x.i,x.al.nombre||x.al.nombreCompleto||'',x.al.cedula||x.al.id||'',x.as.presentes,x.as.ausentes,x.as.tardanzas,`${x.no.entregadas}/${x.no.total}`,x.no.nota===null?'':x.no.nota.toFixed(2),getObs(x.al)]));return '\uFEFF'+lines.map(r=>r.map(v=>{const s=String(v??'');return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s}).join(',')).join('\r\n');}
  function csvAsistencia(){const lines=[['N°','Estudiante','ID','Días asistidos','Inasistencias','Tardanzas']];filas().forEach(x=>lines.push([x.i,x.al.nombre||x.al.nombreCompleto||'',x.al.cedula||x.al.id||'',x.as.presentes,x.as.ausentes,x.as.tardanzas]));return '\uFEFF'+lines.map(r=>r.map(v=>{const s=String(v??'');return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s}).join(',')).join('\r\n');}
  function htmlBoletin(al){const as=asistencia(al),no=notas(al);return`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Boletín - ${esc(al.nombre||al.nombreCompleto||'')}</title><style>body{font-family:Arial,sans-serif;margin:30px;color:#1d2733}.head{background:#234f91;color:#fff;padding:20px;border-radius:14px}table{width:100%;border-collapse:collapse;margin-top:14px}th,td{border:1px solid #cbd5e1;padding:7px;font-size:12px}th{background:#eef4fb}.m{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:15px 0}.m div{padding:10px;background:#eef4fb;border-radius:9px;text-align:center}.m b{display:block;color:#234f91;font-size:18px}</style></head><body><div class="head"><h1 style="margin:0">Boletín de desempeño del lapso</h1><div>${esc(contexto.lapso)} · ${esc(window.profesorActual?.materia||'')}</div></div><p><strong>Estudiante:</strong> ${esc(al.nombre||al.nombreCompleto||'')}<br><strong>Curso:</strong> ${esc(contexto.ano)} · Sección ${esc(contexto.seccion)}<br><strong>Docente:</strong> ${esc(window.profesorActual?.nombre||'')}</p><div class="m"><div><b>${as.presentes}</b>Asistencias</div><div><b>${as.ausentes}</b>Inasistencias</div><div><b>${no.entregadas}/${no.total}</b>Entregadas</div><div><b>${no.nota===null?'—':no.nota.toFixed(2)}</b>Nota /20</div></div><table><thead><tr><th>Actividad</th><th>Fecha</th><th>Entregó</th><th>Nota</th><th>Ponderación</th></tr></thead><tbody>${no.detalle.map(d=>`<tr><td>${esc(d.nombre)}</td><td>${esc(d.fecha||'—')}</td><td>${d.entrego?'Sí':'No'}</td><td>${d.nota===null?'—':d.nota.toFixed(2)}</td><td>${d.ponderacion}%</td></tr>`).join('')||'<tr><td colspan="5">Sin actividades.</td></tr>'}</tbody></table><p><strong>Observación:</strong> ${esc(getObs(al)||'Sin observaciones registradas.')}</p></body></html>`;}
  function actaHtml(){const rows=filas(),prom=rows.filter(x=>x.no.nota!==null),avg=prom.length?prom.reduce((s,x)=>s+x.no.nota,0)/prom.length:null,obs=document.getElementById('cl-obs')?.value.trim()||'';return`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Acta de cierre</title><style>body{font-family:Arial,sans-serif;margin:32px;color:#1d2733}.head{background:#234f91;color:#fff;padding:20px;border-radius:14px}.box{border:1px solid #d9e2ec;border-radius:10px;padding:14px;margin-top:14px}</style></head><body><div class="head"><h1 style="margin:0">Acta resumen de cierre de lapso</h1><div>${esc(contexto.lapso)}</div></div><div class="box"><p><strong>Docente:</strong> ${esc(window.profesorActual?.nombre||'')}<br><strong>Materia:</strong> ${esc(window.profesorActual?.materia||'')}<br><strong>Curso:</strong> ${esc(contexto.ano)} · Sección ${esc(contexto.seccion)} · ${esc(contexto.turno||'')}<br><strong>Estudiantes:</strong> ${rows.length}<br><strong>Actividades:</strong> ${actividades.length}<br><strong>Promedio de sección:</strong> ${avg===null?'—':avg.toFixed(2)} /20<br><strong>Total de inasistencias:</strong> ${rows.reduce((s,x)=>s+x.as.ausentes,0)}</p></div><div class="box"><strong>Observación general</strong><p>${esc(obs||'Sin observaciones generales registradas.')}</p></div><p style="margin-top:35px">Firma del docente: ______________________________</p><p>Recibido por Control de Estudio: ______________________________</p><p style="font-size:11px;color:#667">Generado por EduGestión el ${new Date().toLocaleString('es-ES')}.</p></body></html>`;}

  const crcTable=(()=>{const t=[];for(let i=0;i<256;i++){let c=i;for(let k=0;k<8;k++)c=(c&1)?0xedb88320^(c>>>1):c>>>1;t[i]=c>>>0}return t})();
  function crc32(bytes){let c=0xffffffff;for(const b of bytes)c=crcTable[(c^b)&255]^(c>>>8);return(c^0xffffffff)>>>0}function u16(a,v){a.push(v&255,(v>>>8)&255)}function u32(a,v){a.push(v&255,(v>>>8)&255,(v>>>16)&255,(v>>>24)&255)}
  function zipStore(files){const enc=new TextEncoder(),out=[],central=[];let offset=0;const now=new Date(),dostime=((now.getHours()&31)<<11)|((now.getMinutes()&63)<<5)|((Math.floor(now.getSeconds()/2))&31),dosdate=(((now.getFullYear()-1980)&127)<<9)|(((now.getMonth()+1)&15)<<5)|(now.getDate()&31);files.forEach(f=>{const name=enc.encode(f.name),data=typeof f.data==='string'?enc.encode(f.data):f.data,crc=crc32(data),local=[];u32(local,0x04034b50);u16(local,20);u16(local,0x0800);u16(local,0);u16(local,dostime);u16(local,dosdate);u32(local,crc);u32(local,data.length);u32(local,data.length);u16(local,name.length);u16(local,0);local.push(...name);const start=offset;out.push(...local,...data);offset+=local.length+data.length;const cen=[];u32(cen,0x02014b50);u16(cen,20);u16(cen,20);u16(cen,0x0800);u16(cen,0);u16(cen,dostime);u16(cen,dosdate);u32(cen,crc);u32(cen,data.length);u32(cen,data.length);u16(cen,name.length);u16(cen,0);u16(cen,0);u16(cen,0);u16(cen,0);u32(cen,0);u32(cen,start);cen.push(...name);central.push(...cen)});const centralOffset=offset;out.push(...central);const end=[];u32(end,0x06054b50);u16(end,0);u16(end,0);u16(end,files.length);u16(end,files.length);u32(end,central.length);u32(end,centralOffset);u16(end,0);out.push(...end);return new Blob([new Uint8Array(out)],{type:'application/zip'});}
  function descargar(blob,name){const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),2500);}
  function construirZip(){if(!alumnos.length)throw new Error('Carga primero una sección.');const files=[];if(document.getElementById('cl-inc-cuadro')?.checked)files.push({name:'01_Cuadro_final_notas.csv',data:csvCuadro()});if(document.getElementById('cl-inc-asistencia')?.checked)files.push({name:'02_Resumen_asistencia.csv',data:csvAsistencia()});if(document.getElementById('cl-inc-boletines')?.checked)alumnos.forEach((al,i)=>files.push({name:`03_Boletines/${String(i+1).padStart(2,'0')}_${String(al.nombre||al.nombreCompleto||'Estudiante').replace(/[^a-z0-9áéíóúñü_-]+/gi,'_')}.html`,data:htmlBoletin(al)}));if(document.getElementById('cl-inc-acta')?.checked)files.push({name:'04_Acta_resumen_cierre.html',data:actaHtml()});files.push({name:'LEEME.txt',data:`Paquete de cierre de lapso generado por EduGestión.\nDocente: ${window.profesorActual?.nombre||''}\nMateria: ${window.profesorActual?.materia||''}\nCurso: ${contexto.ano} Sección ${contexto.seccion}\nLapso: ${contexto.lapso}\nFecha: ${new Date().toLocaleString('es-ES')}`});if(!files.length)throw new Error('Selecciona al menos un documento para incluir.');const name=`Cierre_${(contexto.ano||'Curso').replace(/\s+/g,'_')}_Seccion_${(contexto.seccion||'X').replace(/\s+/g,'_')}_${contexto.lapso.replace(/\s+/g,'_')}.zip`;return{blob:zipStore(files),name};}
  function guardarContacto(){const c={email:document.getElementById('cl-email')?.value.trim()||'',whatsapp:document.getElementById('cl-wa')?.value.trim()||''};saveJSON(contactoKey(),c);return c;}
  function generarZip(descargaDirecta){try{guardarContacto();const z=construirZip();if(descargaDirecta)descargar(z.blob,z.name);toast('Paquete de cierre generado correctamente.','success');return z}catch(err){toast(err.message||'No se pudo generar el ZIP.','error');return null}}
  async function compartir(){const z=generarZip(false);if(!z)return;const file=new File([z.blob],z.name,{type:'application/zip'});try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title:'Cierre de lapso',text:`${contexto.lapso} · ${contexto.ano} Sección ${contexto.seccion}`,files:[file]});return;}descargar(z.blob,z.name);toast('Se descargó el ZIP para compartirlo manualmente.','info');}catch(err){if(err?.name!=='AbortError'){descargar(z.blob,z.name);toast('Se descargó el ZIP para compartirlo manualmente.','info');}}}
  function mensaje(){return `Buenos días. Adjunto paquete de cierre de ${contexto.lapso}, ${contexto.ano} Sección ${contexto.seccion}, materia ${window.profesorActual?.materia||''}, docente ${window.profesorActual?.nombre||''}. Incluye cuadro de notas, asistencia, boletines y acta de cierre.`;}
  function abrirCorreo(){const z=generarZip(false);if(!z)return;descargar(z.blob,z.name);const c=guardarContacto();window.location.href=`mailto:${encodeURIComponent(c.email||'')}?subject=${encodeURIComponent(`Cierre de lapso - ${contexto.ano} Sección ${contexto.seccion}`)}&body=${encodeURIComponent(`${mensaje()}\n\nAdjuntar archivo descargado: ${z.name}`)}`;}
  function abrirWhatsApp(){const z=generarZip(false);if(!z)return;descargar(z.blob,z.name);const c=guardarContacto(),phone=String(c.whatsapp||'').replace(/\D/g,'');window.open(`https://wa.me/${phone}?text=${encodeURIComponent(`${mensaje()}\n\nAdjuntar archivo descargado: ${z.name}`)}`,'_blank');}
  function status(msg){const e=document.getElementById('cl-status');if(e)e.textContent=msg;}
  function toast(msg,type='info'){if(typeof window.mostrarToast==='function')window.mostrarToast(msg,type,'Cierre de lapso');else alert(msg);}
  function init(){asegurarUI();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(init,300),{once:true});else setTimeout(init,180);
  window.addEventListener('edugestion:session',()=>setTimeout(()=>{asegurarUI();secciones=[];alumnos=[];stats=null;actividades=[];},100));
})();
...
/* =========================================================
   EduGestión · FASE 16 CORREGIDA
   Historial de cierres para Control de Estudio
   IMPORTANTE: módulo aislado. No modifica login, apiRequest ni conexión.
   ========================================================= */
(() => {
  const MARK='EDUGESTION_HISTORIAL_CIERRES_SAFE_V1';
  if(window[MARK]) return;
  window[MARK]=true;
  const TAB_ID='tab-historial-cierres', SECTION_ID='section-historial-cierres';
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const userKey=()=>String(window.profesorActual?.id||window.profesorActual?.usuario||window.profesorActual?.email||'sin_usuario').replace(/[^a-z0-9_-]/gi,'_');
  const key=()=>`edugestion_historial_cierres_v2_${userKey()}`;
  ...
  function toast(msg){if(typeof window.mostrarToast==='function')window.mostrarToast(msg,'success','Historial de cierres');}
  function contextoActual(){const raw=document.getElementById('cl-seccion')?.value||'';const [ano='',seccion='',turno='']=raw.split('|');return{ano,seccion,turno,lapso:document.getElementById('cl-lapso')?.value||'1er Lapso'};}
  function registrar(accion,medio=''){
    const c=contextoActual(); if(!c.ano||!c.seccion) return;
    const arr=load();
    arr.unshift({id:`hc_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,fecha:new Date().toISOString(),accion,medio,ano:c.ano,seccion:c.seccion,turno:c.turno,lapso:c.lapso,docente:window.profesorActual?.nombre||'',materia:window.profesorActual?.materia||'',enviado:false,fechaEnvio:''});
    save(arr.slice(0,300)); render();
  }
  function estilos(){if(document.getElementById('edu-hc-safe-styles'))return;const st=document.createElement('style');st.id='edu-hc-safe-styles';st.textContent=`
    .hcs-hero{padding:22px;border-radius:22px;background:linear-gradient(135deg,#233876,#316cb0);color:#fff;margin-bottom:18px;box-shadow:0 12px 30px rgba(35,56,118,.18)}.hcs-hero small{font-weight:900;text-transform:uppercase;letter-spacing:.08em;opacity:.92}.hcs-hero h2{margin:6px 0 8px;font-size:1.6rem}.hcs-hero p{margin:0;max-width:980px;opacity:.95}
    .hcs-card{background:var(--card-bg,#fff);border:1px solid var(--border-color,#dfe7ee);border-radius:18px;padding:16px;margin-bottom:14px}.hcs-tools{display:grid;grid-template-columns:2fr 1fr 1fr;gap:9px}.hcs-tools input,.hcs-tools select{width:100%;padding:10px 11px;border:1px solid var(--border-color,#d7e0e8);border-radius:10px;background:var(--input-bg,#fff);color:inherit}.hcs-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;margin:12px 0}.hcs-summary article{padding:12px;border-radius:13px;background:#f1f6fc;border:1px solid #dce9f8}.hcs-summary strong{display:block;font-size:1.2rem;color:#234f91}.hcs-summary span{font-size:.72rem;font-weight:850;opacity:.72}
    .hcs-list{display:grid;gap:10px}.hcs-item{border:1px solid #dce7f4;border-radius:14px;padding:13px;background:var(--card-bg,#fff);display:grid;grid-template-columns:1.6fr auto;gap:12px;align-items:center}.hcs-title{font-weight:900;color:#234f91}.hcs-meta{font-size:.78rem;opacity:.72;margin-top:4px}.hcs-badge{display:inline-flex;padding:4px 8px;border-radius:999px;background:#edf3fb;color:#234f91;font-size:.7rem;font-weight:900;margin-right:4px}.hcs-badge.ok{background:#eaf7ef;color:#227447}.hcs-actions{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.hcs-btn{border:0;border-radius:9px;padding:8px 10px;font-weight:850;cursor:pointer}.hcs-btn.primary{background:#234f91;color:#fff}.hcs-btn.soft{background:#edf3fb;color:#234f91}.hcs-btn.danger{background:#fff0f0;color:#a83232}.hcs-empty{padding:28px;text-align:center;border:1px dashed var(--border-color,#ccd8e5);border-radius:14px;opacity:.75}@media(max-width:800px){.hcs-tools,.hcs-summary{grid-template-columns:1fr}.hcs-item{grid-template-columns:1fr}.hcs-actions{justify-content:flex-start}}
  `;document.head.appendChild(st)}
  function asegurarUI(){
    estilos(); const nav=document.getElementById('app-nav'),main=document.getElementById('app-main'); if(!nav||!main)return;
    let tab=document.getElementById(TAB_ID);
    if(!tab){tab=document.createElement('button');tab.id=TAB_ID;tab.type='button';tab.className='nav-item';tab.setAttribute('aria-selected','false');tab.dataset.title='Historial de cierres';tab.dataset.description='Registro de cierres de Control de Estudio.';tab.innerHTML='<i class="fa-solid fa-clock-rotate-left"></i><span>Historial de cierres</span>';const ref=document.getElementById('tab-estadisticas');nav.insertBefore(tab,ref||null);tab.addEventListener('click',()=>abrir(tab));}
    let sec=document.getElementById(SECTION_ID);
    if(!sec){sec=document.createElement('section');sec.id=SECTION_ID;sec.className='hidden';sec.innerHTML=`<header class="hcs-hero"><small><i class="fa-solid fa-clock-rotate-left"></i> Fase 16 · Control de Estudio</small><h2>Historial de cierres</h2><p>Guarda un registro local por profesor de los cierres generados o preparados para enviar.</p></header><div class="hcs-card"><div class="hcs-tools"><input id="hcs-q" placeholder="Buscar sección, lapso o materia..."><select id="hcs-lapso"><option value="">Todos los lapsos</option><option>1er Lapso</option><option>2do Lapso</option><option>3er Lapso</option></select><select id="hcs-estado"><option value="">Todos los estados</option><option value="enviado">Enviados</option><option value="pendiente">Pendientes</option></select></div><div id="hcs-summary" class="hcs-summary"></div></div><div class="hcs-card"><div id="hcs-list" class="hcs-list"></div></div>`;main.appendChild(sec);['hcs-q','hcs-lapso','hcs-estado'].forEach(id=>sec.querySelector('#'+id)?.addEventListener(id==='hcs-q'?'input':'change',render));}
  }
  function abrir(tab){const sec=document.getElementById(SECTION_ID);if(!sec)return;if(typeof window.cambiarPestana==='function')window.cambiarPestana(tab,sec);else{document.querySelectorAll('#app-nav .nav-item').forEach(x=>{x.classList.toggle('is-active',x===tab);x.setAttribute('aria-selected',x===tab?'true':'false')});document.querySelectorAll('#app-main > section').forEach(x=>x.classList.toggle('hidden',x!==sec));}render();}
  function filtrados(){const q=(document.getElementById('hcs-q')?.value||'').toLowerCase().trim(),lap=document.getElementById('hcs-lapso')?.value||'',est=document.getElementById('hcs-estado')?.value||'';return load().filter(r=>{const t=[r.ano,r.seccion,r.lapso,r.materia,r.docente,r.medio].join(' ').toLowerCase();return(!q||t.includes(q))&&(!lap||r.lapso===lap)&&(!est||(est==='enviado'?r.enviado:!r.enviado));});}
  function render(){const list=document.getElementById('hcs-list'),sum=document.getElementById('hcs-summary');if(!list||!sum)return;const all=load(),arr=filtrados(),env=all.filter(x=>x.enviado).length;sum.innerHTML=`<article><strong>${all.length}</strong><span>Registros totales</span></article><article><strong>${env}</strong><span>Enviados confirmados</span></article><article><strong>${all.length-env}</strong><span>Pendientes</span></article>`;if(!arr.length){list.innerHTML='<div class="hcs-empty">Todavía no hay cierres registrados.</div>';return;}list.innerHTML=arr.map(r=>`<article class="hcs-item" data-id="${esc(r.id)}"><div><div class="hcs-title">${esc(r.ano||'Curso')} · Sección ${esc(r.seccion||'—')} · ${esc(r.lapso||'—')}</div><div class="hcs-meta">${esc(r.materia||'')} · ${esc(r.docente||'')} · ${fmt(r.fecha)}</div><div style="margin-top:7px"><span class="hcs-badge ${r.enviado?'ok':''}">${r.enviado?'Enviado':esc(r.accion||'Generado')}</span>${r.medio?`<span class="hcs-badge">${esc(r.medio)}</span>`:''}</div></div><div class="hcs-actions"><button class="hcs-btn primary" data-act="reabrir">Reabrir</button>${r.enviado?'<button class="hcs-btn soft" data-act="pendiente">Marcar pendiente</button>':'<button class="hcs-btn soft" data-act="enviado">Marcar enviado</button>'}<button class="hcs-btn danger" data-act="eliminar">Eliminar</button></div></article>`).join('');list.querySelectorAll('[data-act]').forEach(b=>b.addEventListener('click',()=>accion(b.closest('.hcs-item')?.dataset.id,b.dataset.act)));}
  function accion(id,act){const arr=load(),i=arr.findIndex(x=>x.id===id);if(i<0)return;const r=arr[i];if(act==='eliminar'){if(!confirm('¿Eliminar este registro del historial?'))return;arr.splice(i,1);save(arr);render();return;}if(act==='enviado'){r.enviado=true;r.fechaEnvio=new Date().toISOString();r.accion='Enviado';save(arr);render();return;}if(act==='pendiente'){r.enviado=false;r.fechaEnvio='';r.accion='Pendiente';save(arr);render();return;}if(act==='reabrir'){document.getElementById('tab-cierre-lapso')?.click();setTimeout(()=>{const lap=document.getElementById('cl-lapso'),sel=document.getElementById('cl-seccion');if(lap)lap.value=r.lapso||'1er Lapso';if(sel){const op=[...sel.options].find(o=>{const [a,s]=String(o.value||'').split('|');return a===r.ano&&s===r.seccion});if(op)sel.value=op.value;}document.getElementById('cl-cargar')?.click();},400);}}
  function instalarCaptura(){if(window.__EDU_HC_CAPTURE__)return;window.__EDU_HC_CAPTURE__=true;document.addEventListener('click',ev=>{const b=ev.target?.closest?.('#cl-zip,#cl-share,#cl-mail,#cl-whatsapp');if(!b)return;setTimeout(()=>{if(b.id==='cl-zip')registrar('Generado');else if(b.id==='cl-share')registrar('Preparado para enviar','Compartir');else if(b.id==='cl-mail')registrar('Preparado para enviar','Correo');else if(b.id==='cl-whatsapp')registrar('Preparado para enviar','WhatsApp');},80);},true);}
  function init(){asegurarUI();instalarCaptura();render();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(init,350),{once:true});else setTimeout(init,220);
  window.addEventListener('edugestion:session',()=>setTimeout(init,150));
})();
/* EDUGESTION_HISTORIAL_CIERRES_SAFE_V1_END */
